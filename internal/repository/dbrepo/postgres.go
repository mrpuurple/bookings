...
// SearchAvailabilityByDatesByRommID returns true if availability exists for roomID, and false if no availability
...
// GetRestrictionsForRommByDate returns restrictions for a room by date range
func (m *postgresDBRepo) GetRestrictionsForRommByDate(roomID int, start, end time.Time) ([]models.RoomRestriction, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
