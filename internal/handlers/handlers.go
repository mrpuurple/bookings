...
import (
	...
	"github.com/mrpuurple/bookings/internal/repository"
	"github.com/mrpuurple/bookings/internal/repository/dbrepo"
)
...
// BookRoom takes URL params, builds a session variable, and takes user to make res screen
...
// PostShowLogin handels logging the user in 
...
func (m *Repository) AdminReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	for _, x := range rooms {
		...
		for d := firstOfMonth; !d.After(lastOfMonth); d = d.AddDate(0, 0, 1) {
			...
			blockedMap[d.Format("2006-01-2")] = 0
		}

		// get all the restrictions for the current room
		restrictions, err := m.DB.GetRestrictionsForRommByDate(x.ID, firstOfMonth, lastOfMonth)
		if err != nil {
			helpers.ServerError(w, err)
			return
		}
		...
		for _, y := range restrictions {
			if y.ReservationID > 0 {
				...
				// It's blocked
				blockedMap[y.StartDate.Format("2006-01-2")] = y.ID
			}
		}
		data[fmt.Sprintf("reservation_map_%d", x.ID)] = reservationMap
		data[fmt.Sprintf("blocked_map_%d", x.ID)] = blockedMap

		m.App.Session.Put(r.Context(), fmt.Sprintf("blocked_map_%d", x.ID), blockedMap)
